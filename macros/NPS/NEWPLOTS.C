...
void NEWPLOTS( Int_t nrun=20990, Int_t nevent = 50000) {
    ...
    TFile *f;
        if (nevent == 50000){
         f= new TFile(Form("../../ROOTfiles/COIN/50k/nps_hms_coin_%i_%i.root",nrun,nevent),"UPDATE");
        }
        else{
         f= new TFile(Form("../../ROOTfiles/COIN/PRODUCTION/nps_hms_coin_%i_-1.root",nrun),"UPDATE");
        }
